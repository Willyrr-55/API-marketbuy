...
const jwt_to_refresh_guard_1 = require("./../guards/jwt-to-refresh.guard");
...
let UsersController = class UsersController {
    ...
    async signUp(res, createUserDto) {
        try {
            ...
            const token = await this.usersService.generateToken({ _id: user['_id'], role: user.role });
            res.status(common_1.HttpStatus.OK).json({ message: 'Te has registrado correctamente', token, user });
        }
        catch (error) {
            console.log(error);
            res.status(common_1.HttpStatus.INTERNAL_SERVER_ERROR).json({ message: 'Ocurrió un error al crear el usuario' });
...
__decorate([
    (0, swagger_1.ApiBearerAuth)('JWT-auth'),
    (0, common_1.UseGuards)(jwt_to_refresh_guard_1.JwtToRefreshGuard),
    ...
    __metadata("design:returntype", Promise)
], UsersController.prototype, "revalidarToken", null);
